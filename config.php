...
function require_login() { if(!is_logged_in()) { header('Location: auth.php?action=login'); exit; } }
function is_admin() { return is_logged_in() && ($_SESSION['user']['role'] === 'admin'); }
function require_admin() { if(!is_admin()) { header('Location: auth.php?action=login'); exit; } }
